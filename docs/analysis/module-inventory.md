...
|:--|:--|:--|--:|:--|:--|
| `entrypoints/background.ts` + background handlers | MV3 bootstrap、单一 121-command registry、automation/sync/chat/export/tool/sandbox orchestration | R4.1 已把 57 个 persistence/library/project/local-preference commands 拆为五组 typed handlers，并抽取接收边界 codec 与 journal mutation composition；根文件仍保留 62 个 transitional cases 和跨域 Chrome IO | 2,690 root LOC + 670 R4.1 handler/composition LOC | Critical | S🔴 U🔴 P🟡 E🔴 R🔴 |
...
| `tests/` | Vitest/jsdom 单元与契约测试 | 145 TS files / 125 test files；sync/cascade fault injection、database migration/convergence、automation cancellation/restart/lease fencing、runtime handler 与 PC capability contracts 已覆盖，但仍没有真实浏览器或 coverage gate | 145 / 27,739 | Medium | S🟢 U🟢 P🟡 E🟡 R🟡 |
...
1. **Runtime messages**
   - 单一 production registry 现拥有 `59 typed / 62 legacy / 2 client-only`；`background.ts` 仅余 62 个 transitional `case`。
   - `core/types.ts` 的 `MessageAction` 有 91 个 variant；32 个 live-only command 仍待 R4.2–R4.4 收敛。
   - R4.1 的 38 个 payload-bearing commands 由一个穷尽 command-codec map 在 handler 接收边界解码；Memory、Skill、Preset 复用各自领域 codec，明确兼容例外在依赖调用前完成检查。剩余 legacy surface 仍有 32 个直接 `as` 强转和九个 delegated payload readers。
...
- **Public API**：121 个 live runtime commands 的单一入口；59 个 typed handlers，62 个 transitional legacy cases。
- **Internal dependencies**：Memory、Skill、MCP、tool、sync、automation、DeepSeek、export、sandbox、browser control 等几乎全部领域。
- **Transformation note**：沿现有 registry 按 R4.2–R4.4 的既定 ownership 继续迁移；每组迁移后立即删除对应旧 case，禁止增加第二套路由或 validator。
- **S.U.P.E.R issue**：五项仍红；R4.1 已移出 persistence/library/project/local-preference 业务流程，但根文件仍承载 62 个命令和多域 lifecycle/IO。
