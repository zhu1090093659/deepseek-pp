...
本清单基于 2026-07-13 当前工作区，排除 `node_modules/`、`dist/`、`docs/archives/` 和 Android 生成资产。`core/skill/*-official/` 中约 1.1 MB 的 bundled Skill 文档作为资源记录，不计入 TypeScript LOC。
...
| `automation` | Schedule、runner、run storage、locking/retry/timeout；`runAutomation`, `runDeepSeekAutomation` | DeepSeek adapter、prompt、tool-loop、Chrome storage | 7 / 1,642 | High | S🟢 U🟡 P🟢 E🔴 R🟡 |
...
| `deepseek` | Web API adapter、PoW、upload、history、SSE、official API、export transport | fetch、Chrome storage、page localStorage、WASM、interceptor contracts | 5 / 1,482 | High | S🔴 U🟡 P🟡 E🔴 R🔴 |
...
| `mcp` | MCP client/discovery/store 和 HTTP/SSE/bridge/native transports | JSON-RPC port 清晰；permission/header/store 与 Chrome 实现耦合 | 12 / 2,232 | High | S🟢 U🟡 P🟢 E🟡 R🟢 |
| `memory` | Dexie store/migrations、scope、selector、injector、importer/tool | Dexie、root types、prompt/i18n | 6 / 574 | Medium | S🟢 U🟡 P🟡 E🟡 R🟡 |
...
| `platform` | Capability matrix、`PlatformServices`、browser implementation | 抽象存在但生产接入很少；仍有 69 个 core/entrypoint 文件直用 `chrome.*` | 5 / 298 | High-risk | S🟢 U🟡 P🔴 E🟡 R🔴 |
| `preset` | Prompt preset CRUD | Chrome storage、root types | 1 / 62 | Low | S🟢 U🟢 P🟡 E🔴 R🔴 |
| `project` | Project/conversation binding、pending context、schema v2 | Chrome storage；normalize/schema 较清晰 | 4 / 383 | Medium | S🟢 U🟢 P🟢 E🔴 R🟡 |
...
| `saved-items` | Saved prompt/bookmark CRUD、schema v1 | Chrome storage；legacy array normalize | 3 / 176 | Low | S🟢 U🟢 P🟢 E🔴 R🟡 |
...
| `skill` | Built-ins、registry、GitHub/local import、creator tool、bundled resources | Chrome storage、fetch/GitHub、MCP、Shell；两个 importer 重复解析/policy | 8 / 3,119 + resources | Critical | S🔴 U🟡 P🟡 E🔴 R🔴 |
| `sync` | Config、schema、OAuth、WebDAV/GDrive/OneDrive backends | `StorageBackend` port 良好；factory 与 provider type 形成循环，identity/storage 固定 | 8 / 1,101 | High | S🟢 U🟡 P🟢 E🔴 R🟢 |
| `theme` | DeepSeek theme store | Chrome storage | 1 / 16 | Low | S🟢 U🟢 P🟡 E🔴 R🔴 |
| `token` | Token 粗略估算 | 纯函数 | 1 / 13 | Low | S🟢 U🟢 P🟢 E🟢 R🟢 |
| `tool` | Tool types/catalog/runtime/restore/history、内置 providers | 依赖 artifact/browser-control/MCP/memory/project/skill；硬编码 dispatch | 13 / 2,079 | Critical | S🔴 U🔴 P🟡 E🔴 R🔴 |
...
|:--|:--|:--|--:|:--|:--|
| `entrypoints/background.ts` | MV3 bootstrap、119 个 runtime commands、automation/sync/chat/export/tool/sandbox orchestration | Router、payload cast、domain orchestration 和 Chrome IO 集中 | 2,601 LOC | Critical | S🔴 U🔴 P🔴 E🔴 R🔴 |
...
| Android adapter | WebView activity、JavaScriptInterface、Chrome shim | 浏览器 contract 的不完整复制，存在 origin/storage 安全边界 | ~352 native LOC | Critical | S🟡 U🟡 P🔴 E🔴 R🔴 |
...
| `scripts/` | CI/release/manifest/i18n/smoke/Android staging | 文件职责大体单一；部分 contract 是字符串断言 | 18 / 3,281 | Medium | S🟢 U🟢 P🟡 E🟡 R🟢 |
| `tests/` | Vitest/jsdom 单元与契约测试 | 覆盖广但没有真实浏览器、migration、fault injection 或 coverage gate | 63 / 10,802 | Medium | S🟢 U🟢 P🟡 E🟡 R🟡 |
...
- 文件图中存在两个强连通分量：
  ...
  2. 3 文件 SCC：`sync/storage-backend.ts` 与 GDrive/OneDrive clients。Port 与 composition factory 位于同一文件，使 provider 反向 type-import factory 定义的 interface。
...
1. **Runtime messages**
   ...
   - Android bridge 维护另一套字符串分派子集。

2. **Platform abstraction**
   - `PlatformServices` 已存在，但生产代码几乎没有使用 `createBrowserExtensionPlatformServices()`。
   - 69 个 core/entrypoint 文件直接访问 `chrome.*`，其中约 40 个位于 core。
...
- **Public API**：`StorageBackend`, `createStorageBackend`, schema validators。
- **Transformation note**：拆开 port 与 factory；在 provider 抽象之上增加 generation manifest、staging、commit/rollback。
