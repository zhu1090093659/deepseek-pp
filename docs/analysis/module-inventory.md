...
| `project` | Project/conversation binding、lossless v1→v2 codec、pending context、project/Memory cascade | Versioned repository、Memory store、shared persistence lock；local/sync/UI 复用单一 codec | 5 / 642 | Medium | S🟢 U🟢 P🟢 E🔴 R🟢 |
...
7. **Persistence**
   ...
   - R3.5 已让 Project、Saved Items、Scenario 共用窄 storage slot/versioned repository，领域 codec 成为 local/sync/UI 唯一验证来源；Project/Memory cascade 与跨 MV3 realm Scenario mutation 仍分别由 R3.6/R4.4 负责。
...
- **Transformation note**：T2.4 已拆开远端 port/factory 并增加 generation pointer commit；T2.5 已增加纯 local-apply coordinator、raw browser preimage adapter、独立恢复 journal 与 background recovery barrier；R3.5 已统一 Project/Saved/Scenario codec/repository。R3.6–R3.8 与 R3.10 (#356–#358、#380) 继续处理数据库 truth、cross-key mutation 和并发正确性，R6.5 / #379 只处理测量后的写放大。
