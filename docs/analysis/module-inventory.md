...
本清单基于 2026-07-13 Issue #320 工作树，排除 `node_modules/`、`dist/`、`docs/archives/` 和生成资产。`core/skill/*-official/` 中约 1.1 MB 的 bundled Skill 文档作为资源记录，不计入 TypeScript LOC。Android 支持面已删除。
...
|:--|:--|:--|--:|:--|:--|
| `entrypoints/background.ts` | MV3 bootstrap、119 个 runtime commands、automation/sync/chat/export/tool/sandbox orchestration | Router、payload cast、domain orchestration 和 Chrome IO 集中；sync 本地 apply 已移出，仅保留 lifecycle barrier/composition | 2,813 LOC | Critical | S🔴 U🔴 P🟡 E🔴 R🔴 |
...
| `tests/` | Vitest/jsdom 单元与契约测试 | 106 TS files / 92 test files；sync 已有完整本地/远端 fault injection，但仍没有真实浏览器、完整 domain migration 或 coverage gate | 106 / 18,026 | Medium | S🟢 U🟢 P🟡 E🟡 R🟡 |
