...
本清单基于 2026-07-13 Issue #319 工作树，排除 `node_modules/`、`dist/`、`docs/archives/` 和生成资产。`core/skill/*-official/` 中约 1.1 MB 的 bundled Skill 文档作为资源记录，不计入 TypeScript LOC。Android 支持面已删除。
...
| `sync` | Config、snapshot、generation、schema、OAuth、WebDAV/GDrive/OneDrive backends | `StorageBackend` port 零具体实现依赖；composition factory 与 generation pipeline 分离，identity/storage 固定 | 12 / 1,519 | High | S🟢 U🟢 P🟢 E🔴 R🟢 |
...
- **Transformation note**：T2.4 已拆开 port 与 factory，并在 provider 抽象之上增加 generation manifest、staging 和 pointer commit；T2.5 继续处理本地 apply journal/rollback。
