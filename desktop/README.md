...
> and an app icon are not configured yet — add `build.win.icon` / `build.mac.icon`
> (≥512px) and signing credentials before public distribution.
...
- `contextIsolation:false` is used so the polyfill is visible to injected
  scripts. Before shipping, move to a `contextBridge`-exposed polyfill.
